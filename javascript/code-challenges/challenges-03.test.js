...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let newForbiddenValues = arr.filter((element) => !forbiddenValues.includes(element));
  return newForbiddenValues;
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  arr.filter((element) => {
    if (element.baseStat > minBaseStat) {
      return newArr.push(element.stat.name);
    }
  });
  return newArr;
};
...
const evenOddNumericValues = (arr) => {
  ...
  newArr.map((element) => {
    if (element % 2 === 0) {
      return evenOrOdd.push("even");
    } else return evenOrOdd.push("odd");
  });
  return evenOrOdd;
};
