...
const toLastNames = people => {
  ...
  let newPeople = [];
  people.map((elements) => {
    return newPeople.push(`${elements.firstName} ${elements.lastName}`);
  });
  return newPeople;
};
...
const addPurchases = (arr) => {
  // Solution code here...
  let newPurchases = arr.reduce((x, z) => {
    return x + z.purchasePrice;
  }, 0);
  return newPurchases;
};
