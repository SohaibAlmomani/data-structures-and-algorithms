...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let divisible = input.map((elements) => {
    return elements
      .filter((value) => {
        return typeof value === "number" && value % 5 === 0;
      })
      .map((result) => Math.pow(2, result));
  });
  return divisible;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let gender = data
    .filter((value) => value.gender === "male" || value.gender === "female")
    .map((val) => val.name)
    .join(" and ");
...
let findShortest = (data) => {
  ...
  return data.reduce((min, value) => {
    if (Number(min.height) < Number(value.height)) {
      return min;
    } else {
      return value;
    }
