...
const replaceZeros = (string) => {
  // Solution code here...
  let replace = string.replace(/0/g, "zero");
  return replace;
};
...
const validateEmail = (email) => {
  // Solution code here...
  let regExRules = /^\w+\.?\w+@\w+.(com|net|org)$/;
  return regExRules.test(email);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let acceptsRegEx = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
  return acceptsRegEx.test(phoneNumber);
};
