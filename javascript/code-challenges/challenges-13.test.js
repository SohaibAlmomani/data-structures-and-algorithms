...
const longestString = (arr) => {
  ...
  let finalResult = 0;
  if (arr.length > 0) {
    let newResult = arr.reduce((x, y) => {
      return x.length > y.length ? x : y;
    });
    finalResult = arr.indexOf(newResult);
  } else {
    finalResult = -1;
  }
  return finalResult;
...
const firstLetters = (arr) => {
  // Solution code here...
  let output = arr.map((letter) => letter[0]);
  return output;
};
...
const findHappiness = (arr) => {
  // Solution code here...
  let output = arr.filter((str) => str.includes(":)"));
  return output;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let output = arr.map((e) => e.replace(/\D/g, ""));
  return output;
};
...
const allHappy = (arr) => {
  // Solution code here...
  let output =
    arr.filter((output) => !output.includes(":)")).length > 0 ? false : true;
  return output;
};
